...
const chartData = computed(() => ({
    labels: props.data.map((i) => i.branch_name),
    datasets: [
        {
            label: 'Revenue',
        ...
        <CardContent class="space-y-4">
            ...
            <div v-if="data.length" class="rounded-md border">
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        <tr v-for="item in data" :key="item.branch_id" class="border-b last:border-0">
                            <td class="px-4 py-2 font-medium">{{ item.branch_name }}</td>
