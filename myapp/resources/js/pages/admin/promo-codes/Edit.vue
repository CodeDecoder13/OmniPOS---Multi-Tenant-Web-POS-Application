...
import type { BreadcrumbItem, PromoCode } from '@/types';
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit', href: `/admin/promo-codes/${props.promoCode.id}/edit` },
];
...
const form = useForm({
    ...
    max_uses: props.promoCode.max_uses ?? (undefined as number | undefined),
    valid_from: props.promoCode.valid_from ? props.promoCode.valid_from.substring(0, 10) : '',
    valid_until: props.promoCode.valid_until ? props.promoCode.valid_until.substring(0, 10) : '',
    is_active: props.promoCode.is_active,
    applicable_plans: props.promoCode.applicable_plans ?? ([] as string[]),
});
...
function submit() {
    ...
    })).put(`/admin/promo-codes/${props.promoCode.id}`);
}
    ...
    <AdminLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-6 p-6">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-bold">Edit Promo Code</h1>
                <div class="text-sm text-muted-foreground">
                    Used {{ promoCode.used_count }} time{{ promoCode.used_count !== 1 ? 's' : '' }}
                </div>
            </div>
