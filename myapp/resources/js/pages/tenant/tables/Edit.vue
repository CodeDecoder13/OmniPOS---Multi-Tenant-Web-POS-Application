...
const form = useForm({
    ...
    sort_order: props.table.sort_order,
...
function onStatusChange(value: string | number | boolean | Record<string, string>) {
    form.status = String(value) as Table['status'];
}

function submit() {
    form.put(tenantUrl(`tables/${props.table.id}`));
}

const breadcrumbs = [
    { title: 'Tables', href: tenantUrl('tables') },
    { title: 'Edit', href: tenantUrl(`tables/${props.table.id}/edit`) },
];
</script>

<template>
    <TenantLayout title="Edit Table" :breadcrumbs="breadcrumbs">
        <div class="mx-auto max-w-2xl space-y-6">
            ...
            <form @submit.prevent="submit" class="space-y-4">
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <Button type="submit" :disabled="form.processing">Update Table</Button>
                </div>
            </form>
        </div>
    </TenantLayout>
