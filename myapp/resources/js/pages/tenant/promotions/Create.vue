...
function submit() {
    ...
    })).post(tenantUrl('promotions'));
}

const breadcrumbs = [
    { title: 'Promotions', href: tenantUrl('promotions') },
    { title: 'Create', href: tenantUrl('promotions/create') },
];
</script>

<template>
    <TenantLayout title="Create Promotion" :breadcrumbs="breadcrumbs">
            ...
            <form @submit.prevent="submit" class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        ...
                        <Input id="code" v-model="form.code" placeholder="e.g. WELCOME20" class="uppercase" :class="{ 'border-destructive': form.errors.code }" />
                        <p v-if="form.errors.code" class="text-sm text-destructive">{{ form.errors.code }}</p>
                    </div>
                    <div class="space-y-2">
                        <Label for="name">Name *</Label>
                        <Input id="name" v-model="form.name" placeholder="e.g. Welcome Discount" :class="{ 'border-destructive': form.errors.name }" />
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <Button type="submit" :disabled="form.processing">Create Promotion</Button>
                </div>
            </form>
        </div>
    </TenantLayout>
