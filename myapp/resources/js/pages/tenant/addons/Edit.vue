...
const form = useForm({
    ...
    category_label: props.addon.category_label ?? '',
    is_active: props.addon.is_active,
    sort_order: props.addon.sort_order,
});

function submit() {
    form.put(tenantUrl(`addons/${props.addon.id}`));
}

const breadcrumbs = [
    { title: 'Add-ons', href: tenantUrl('addons') },
    { title: 'Edit', href: tenantUrl(`addons/${props.addon.id}/edit`) },
];
</script>

<template>
    <TenantLayout title="Edit Add-on" :breadcrumbs="breadcrumbs">
        <div class="mx-auto max-w-lg space-y-6">
            <h1 class="text-2xl font-bold">Edit Add-on</h1>
            ...
            <form @submit.prevent="submit" class="space-y-4">
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <Button type="submit" :disabled="form.processing">Update Add-on</Button>
                </div>
            </form>
        </div>
    </TenantLayout>
