...
import type { PurchaseOrder, Supplier, Branch, Product } from '@/types';
...
function onProductSelect(index: number, productId: string | number | boolean | Record<string, string>) {
    ...
    if (product?.cost_price && !form.items[index].unit_cost) {
...
function submit() {
    form.put(tenantUrl(`purchase-orders/${po.id}`));
}

const breadcrumbs = [
    { title: 'Purchase Orders', href: tenantUrl('purchase-orders') },
    { title: po.po_number, href: tenantUrl(`purchase-orders/${po.id}`) },
    { title: 'Edit', href: tenantUrl(`purchase-orders/${po.id}/edit`) },
];
</script>

<template>
    <TenantLayout title="Edit Purchase Order" :breadcrumbs="breadcrumbs">
        <div class="mx-auto max-w-3xl space-y-6">
            <h1 class="text-2xl font-bold">Edit {{ po.po_number }}</h1>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="flex justify-end gap-3">
                    <Button variant="outline" type="button" as-child>
                        <a :href="tenantUrl(`purchase-orders/${po.id}`)">Cancel</a>
                    </Button>
